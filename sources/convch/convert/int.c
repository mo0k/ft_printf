...
/*   Updated: 2017/09/18 22:37:05 by mo0ky            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void convert_int(t_fmt *fmt, t_buffer_static *sbuff, va_list *args, int *ret)
{
	...
	int diff;
	state = 0;
	if (!(ptr = init_var_signed(fmt, args, test, 10)))
		return ;
	nbrlen = fmt->len;
	//printf("ptr:%s, nbrlen:%d\n", ptr, nbrlen);
	if (fmt->flag_char[4] && fmt->type.ll >= 0)
		do_space(sbuff, fmt->width, &nbrlen, ret);
	if (fmt->precision == -1 && fmt->width == -1)
		//return (more_speed(fmt, sbuff, multi, ret));
	...
	if ((fmt->flag_char[1] && fmt->precision == -1) || fmt->flag_char[2])
		if ((state = 1) && do_sign(fmt, sbuff, fmt->type.ll, ret) == 2
			...
			nbrlen++;
	if (fmt->flag_char[2])
		//*ret = do_preci_and_fillnbr(fmt, sbuff, multi, nbrlen);
	...
	if (!fmt->flag_char[2])
		//*ret = do_preci_and_fill(fmt, sbuff, multi, nbrlen);
		*ret = do_preci_and_fill(fmt, sbuff, ptr, 1);
}
